...
def download_study_info(
    nct_id: str, delay: float = 1.0, session: Union[requests.Session, None] = None
) -> bool:
    """
    ...
    if response.status_code != 200:
        logging.error(
            "Error: received status code %s for trial %s", response.status_code, nct_id
        )
        time.sleep(delay)
        return False
    ...
    if local_root is not None:
        for field in fields_to_check:
            ...
            if local_elem is not None and online_elem is not None:
                local_text = normalize_whitespace(
                    ET.tostring(local_elem, encoding="unicode").strip()
                )
                online_text = normalize_whitespace(
                    ET.tostring(online_elem, encoding="unicode").strip()
                )
...
def parallel_downloader(
    nct_ids: List[str], n_jobs: int = 10, delay: float = 1.0
) -> List[bool]:
    """
    ...
    results = joblib.Parallel(n_jobs=n_jobs)(
        joblib.delayed(download_wrapper)(nct_id)
        for nct_id in tqdm(nct_ids, desc="Downloading trials")
    )
    return results
...
class Downloader:
    ...
    def download_and_update_trials(self) -> List[bool]:
        ...
        results = parallel_downloader(
            self.id_list, n_jobs=self.n_jobs, delay=self.delay
        )

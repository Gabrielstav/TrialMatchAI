...
class ModelArguments:
    ...
    lora_rank: int = field(
        default=64, metadata={"help": "The rank dimension for LoRA."}
    )
    lora_alpha: float = field(
        default=16, metadata={"help": "The scaling factor (alpha) for LoRA."}
    )
    lora_dropout: float = field(
        default=0.05, metadata={"help": "The dropout rate for LoRA layers."}
    )
    ...
    save_merged_lora_model: bool = field(
        default=False,
        metadata={
            "help": "If True, merges the LoRA parameters into the base model before saving."
        },
    )
    use_flash_attn: bool = field(
        default=True,
        metadata={
            "help": "If True, use flash attention during training (if supported)."
        },
    )
    use_slow_tokenizer: bool = field(
        default=False,
        metadata={
            "help": "If True, use a slow (Python-based) tokenizer instead of a fast (C++/Rust) one."
        },
    )
    low_cpu_mem_usage: bool = field(
        default=False,
        metadata={
            "help": "If True, create the model as an empty shell and then load weights, reducing RAM usage."
        },
    )
    cache_dir: str = field(
        default="tmp",
        metadata={
            "help": "Path to the directory where models and tokenizers are cached."
        },
    )
    token: str = field(
        default=None, metadata={"help": "HuggingFace hub token for private models."}
    )
    from_peft: str = field(
        default=None,
    ...
    lora_extra_parameters: str = field(
        default=None, metadata={"help": "Additional modules to save when using LoRA."}
    )
...
class DataArguments:
    ...
    load_from_disk: bool = field(
        default=False,
        metadata={
            "help": "If True, load a previously saved dataset from disk instead of processing from scratch."
        },
    )
    ...
    load_disk_path: str = field(
        default=None,
        metadata={
            "help": "Path to the saved dataset on disk if load_from_disk is True."
        },
    )

    save_to_disk: bool = field(
        default=False, metadata={"help": "If True, save the processed dataset to disk."}
    )

    save_disk_path: str = field(
        default=None,
        metadata={
            "help": "Path to save the processed dataset if save_to_disk is True."
        },
    )
    ...
    def __post_init__(self):
        if not os.path.exists(self.train_data):
            raise FileNotFoundError(
                f"Cannot find file: {self.train_data}. Please provide a valid path."
            )

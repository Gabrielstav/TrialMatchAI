...
class TrainDataset(Dataset):
    def __init__(self, args, tokenizer: PreTrainedTokenizer):
            ...
            for file in os.listdir(args.train_data):
                ...
                if len(temp_dataset) > args.max_example_num_per_dataset:
                    temp_dataset = temp_dataset.select(
                        random.sample(
                            range(len(temp_dataset)), args.max_example_num_per_dataset
                        )
                    )
        ...
        messages = [
            {
                "role": "system",
                "content": "You are an expert trained on healthcare and biomedical domain!{instruction}",
            },
...
class DataCollatorForFinetuning(DataCollatorForSeq2Seq):
    ...
    def __post_init__(self):
        ...
        if self.tokenizer.pad_token_id != 128009:
            raise ValueError(
                "The tokenizer pad_token_id must be set to 128009 for LLaMA."
            )
    ...
    def __call__(
        ...
        labels_tensor[labels_tensor == self.tokenizer.pad_token_id] = (

...
class EvaluationScript:
    def __init__(
        self, model_path, adapter_path=None, devices=None, torch_dtype=torch.float16
    ):
    ...
    def _initialize_token_ids(self):
        ...
        self.match_token_id, self.mismatch_token_id, self.neutral_token_id = [
    ...
    def preprocess_text(self, text):
        ...
        statement_a = (
            unicodedata.normalize("NFKD", match_a.group(1).strip()) if match_a else ""
        )
        statement_b = (
            unicodedata.normalize("NFKD", match_b.group(1).strip()) if match_b else ""
        )
    ...
    def generate_predictions(self, dataset):
        ...
        workers = [
            ctx.Process(
                target=self.process_on_device, args=(device, input_queue, output_queue)
            )

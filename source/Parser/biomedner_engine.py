...
def process_text_batch(text_batch):
    ...
    if bio_med_ner is None:
        raise RuntimeError(
            "bio_med_ner is not initialized. Please call init_worker first."
        )
    results = []
...
class BioMedNER:
    ...
    def tag_entities(self, text, base_name, biomedner_home):
        n_ascii_letters = sum(1 for char in text if char in string.ascii_letters)
    ...
    async def async_ner(self, arguments):
        ...
        return {k: v for e in result if e is not None for k, v in e.items()}

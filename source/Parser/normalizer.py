...
class Normalizer:
    def __init__(
        ...
        self.NORM_DICT_PATH = {
            ...
            "sign symptom": os.path.join(
                self.BASE_DIR, "dictionary/dict_SignSymptom.txt"
            ),
        }
        ...
        with ThreadPoolExecutor() as executor:
            future_dict = {
                "chemical": executor.submit(
                    ChemicalNormalizer, self.NORM_DICT_PATH["drug"]
                ),
                "species": executor.submit(
                    SpeciesNormalizer, self.NORM_DICT_PATH["species"]
                ),
                "cellline": executor.submit(
                    CellLineNormalizer, self.NORM_DICT_PATH["cell line"]
                ),
                "celltype": executor.submit(
                    CellTypeNormalizer, self.NORM_DICT_PATH["cell type"]
                ),
                "procedure": executor.submit(
                    ProcedureNormalizer, self.NORM_DICT_PATH["procedure"]
                ),
                "sign symptom": executor.submit(
                    SignSymptomNormalizer, self.NORM_DICT_PATH["sign symptom"]
                ),
            }
        ...
        if self.use_neural_normalizer:
            with ThreadPoolExecutor() as executor:
                future_dict = {
                    "neural_disease": executor.submit(
                        NeuralNormalizer,
                        model_name_or_path=self.NEURAL_NORM_MODEL_PATH["disease"],
                        cache_path=self.NEURAL_NORM_CACHE_PATH["disease"],
                        no_cuda=no_cuda,
                    ),
                    "neural_sign symptom": executor.submit(
                        NeuralNormalizer,
                        model_name_or_path=self.NEURAL_NORM_MODEL_PATH["sign symptom"],
                        cache_path=self.NEURAL_NORM_CACHE_PATH["sign symptom"],
                        no_cuda=no_cuda,
                    ),
                    "neural_chemical": executor.submit(
                        NeuralNormalizer,
                        model_name_or_path=self.NEURAL_NORM_MODEL_PATH["drug"],
                        cache_path=self.NEURAL_NORM_CACHE_PATH["drug"],
                        no_cuda=no_cuda,
                    ),
                    "neural_gene": executor.submit(
                        NeuralNormalizer,
                ...
                self.neural_sign_symptom_normalizer = future_dict[
                    "neural_sign symptom"
                ].result()
                self.neural_chemical_normalizer = future_dict[
                    "neural_chemical"
                ].result()
    ...
    def neural_normalize(self, ent_type, tagged_docs):
        ...
        if ent_type == "disease":
            ...
            norm_entities = self.neural_sign_symptom_normalizer.normalize(
                names=cuiless_entity_names
            )
    ...
    def run_normalizers_wrap(self, ent_type, base_name, names, saved_items, results):
        results.append(
            (ent_type, self.run_normalizer(ent_type, base_name, names, saved_items))
        )
    ...
    def run_normalizer(self, ent_type, base_name, names, saved_items):
        ...
        if ent_type in ["disease"]:
            ...
            norm_abs_path = os.path.join(
                self.NORM_INPUT_DIR[ent_type], base_thread_name + ".txt"
            )
            ...
            norm_out_path = os.path.join(
                self.NORM_OUTPUT_DIR[ent_type], output_filename
            )
            if os.path.exists(norm_out_path):
                with open(norm_out_path, "r") as norm_out_f:
                    for line in norm_out_f:
                        oid = line[:-1]
                        oids.append(
                            oid if oid != self.NO_ENTITY_ID else self.NO_ENTITY_ID
                        )
            ...
            with (
                open(norm_inp_path, "w") as norm_inp_f,
                open(norm_abs_path, "w") as norm_abs_f,
            ):
                for saved_item in saved_items:
                    ...
                    ent_names = [
                        abstract_title[loc["start"] : loc["end"]] for loc in entities
                    ]
